...
class TokenService {
   ...
   */
  generateAccessToken(user: { id: string; email: string; role: 'trainer' | 'client' | 'admin' }): string {
  ...
  async verifyRefreshToken(token: string): Promise<{ userId: string; sessionId: string }> {
    ...
    if (!session) {
      logger.warn('Invalid or expired refresh token', { tokenHash: tokenHash.substring(0, 20) + '...' });
  ...
  async blacklistToken(tokenId: string): Promise<void> {
    ...
    await redis.setEx(key, expiry, 'true');
