...
class ExerciseDetailService {
  static async getExerciseDetail(id: string) {
    const exercise = await mockedPrisma.exercise.findUnique({ where: { id } });
...
describe('FORGE: Story 004-04 - View Exercise Details', () => {
  ...
  it('returns exercise details', async () => {
    mockedPrisma.exercise.findUnique.mockResolvedValue({ id: 'ex-1', name: 'Bench Press', bodyPart: 'chest' });
